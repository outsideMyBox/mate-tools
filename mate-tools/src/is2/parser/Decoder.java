...
final public class Decoder {
	...
	public static Parse decode(short[] pos, DataFES x, boolean projective, boolean training) throws InterruptedException {
		...
		ParallelDecoders pds = new ParallelDecoders();

		for (int i = 0; i < Parser.THREADS; i++) pds.add(pos, x, O, C, n);
		...
		for (short k = 1; k < n; k++) {
			...
			executerService.invokeAll(pds.getParallelDecoders());
		}
	...
	public static void rearrange(short[] pos, short[] heads, short[] labs, DataFES x, boolean training) throws InterruptedException {
		...
		ParallelRearranges prs = new ParallelRearranges();
		...
		while (true) {
			...
			prs.clear();
			for (int i = 0; i < threads; i++) prs.add(isChild, pos, x, heads, labs);
			...
			for (int ch = 1; ch < heads.length; ch++) {
				...
				for (short pa = 0; pa < heads.length; pa++) {
					...
					prs.addInParentsAndOrder(p, (short) ch, pa);
				}
			}
			executerService.invokeAll(prs.getParallelRearranges());

			for (ParallelRearrange.PA rp : prs.getOrder())
				if (max < rp.max) {
					max = rp.max;
					wh = rp.wh;
					nPar = rp.nPar;
